...
public class CacheTemplate {
     ...
     * @return
     */
    public <T> T get(String key, TypeReference<T> clazz, int expire, LoadDataCallback loadDataCallback) {
        Jedis jedis = pool.getResource();
        try {
            ...
            synchronized (key) {
                ...
                log.info("================== no Cache");
                jsonStr = loadDataCallback.load();
                if (jsonStr != null) {
                    jedis.setex(key, expire, jsonStr);
                    return JSON.parseObject(jsonStr, clazz);
                }
            }
        } finally {
  ...
  private CacheTemplate cacheTemplate = new CacheTemplate();
    ...
    public Map<Integer, Object> query() {
        return cacheTemplate.get(
                ...
                1,
                new CacheTemplate.LoadDataCallback() {
                    @Override
                    public String load() {
                        return JSON.toJSONString(getDBBookMap());
                    }
                });
    }
```
